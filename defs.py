...
class defs():
    def __init__(self, path: str, insitu_path: str, n0: float, iteration: int, recovery: bool, check = True):
        self.n0 = n0 # cm^-3
    ...
    def EDensitySim(self, normalized: bool):
        """
    ...
    def bunchMask(self, recovery: bool) -> tuple:
        i = self.iteration
    ...
    def quickEfficiency(self) -> float:
        """
    ...
    def getProfile(self, plot = False):
        i = self.iteration
        ...
        nD = plt.hist(self.zd, bins = len(Ez[maskD]), facecolor = 'r', linewidth = 0.2, weights = self.wd)[0]
        nW = plt.hist(self.zw, bins = (len(Ez[maskW])), facecolor = 'b', linewidth = 0.2, weights = self.ww)[0]
        if self.recovery:
            # zr, wr = self.ts.get_particle(species = 'recovery', iteration = i, var_list = ['z', 'w'])
            nR = plt.hist(self.zr, bins = (len(Ez[maskR])), facecolor = 'g', linewidth = 0.2, weights = self.wr)[0]
        else:
            nR = 0
        plt.close()

        if plot:
            plt.plot(self.info.z[maskD], nD, 'r', alpha = .5)
            plt.plot(self.info.z[maskW], nW, 'r', alpha = .5)
            plt.plot(self.info.z[maskR], nR, 'r', alpha = .5)


        return nD, self.info.z[maskD], nW, self.info.z[maskW], nR, self.info.z[maskR]
